  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto">
            ...
            <div className="lg:col-span-5 flex flex-col justify-between items-start lg:items-end">
              <div className="space-y-8 md:space-y-12 text-left lg:text-right w-full">
                <div className="flex flex-col space-y-3 md:space-y-4">
                  <a href="#mision" className="text-left lg:text-right text-xl md:text-2xl font-serif italic hover:text-gold transition-colors">
                    Nuestra Misión
                  </a>
                  <a href="#eventos" className="text-left lg:text-right text-xl md:text-2xl font-serif italic hover:text-gold transition-colors">
                    Próximos Eventos
                  </a>
                </div>
          ...
          <div className="pt-12 md:pt-24 border-t border-black/5 w-full flex flex-col md:flex-row justify-between items-center text-[10px] text-gray-400 uppercase tracking-[0.4em] font-bold gap-6 md:gap-0">
            ...
            <div className="flex gap-8 md:gap-12">
              ...
              <button onClick={() => setShowInfo(true)} className="hover:text-gold transition-colors cursor-pointer">Privacidad</button>
              <button onClick={() => setShowInfo(true)} className="hover:text-gold transition-colors cursor-pointer">Comunidad</button>
            </div>
          </div>
        </div>
      </footer>
      ...
      <Modal isOpen={showInfo} onClose={() => setShowInfo(false)}>
        <div className="bg-alabaster rounded-3xl p-10 border border-white/80 shadow-2xl text-center">
          <div className="w-16 h-px bg-gold/40 mx-auto mb-8" />
          <h3 className="font-serif text-3xl md:text-4xl text-charcoal mb-6">Información Legal</h3>
          <div className="text-gray-600 text-base leading-relaxed font-light space-y-4">
            ...
            <span className="block">Todos los contenidos publicados en este sitio son de carácter informativo y formativo. Para cualquier consulta, escríbenos a <a href={`mailto:${config.contact.email}`} className="text-gold hover:underline">{config.contact.email}</a>.</span>
