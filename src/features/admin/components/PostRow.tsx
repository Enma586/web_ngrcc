...
export function PostRow({ post, onEdit, onDelete }: Props) {
  ...
  return (
    <>
      <div className="group flex items-center justify-between p-5 md:p-7 rounded-3xl border border-transparent hover:bg-white hover:shadow-2xl hover:shadow-charcoal/8 hover:border-eggshell cursor-pointer post-row-transition">
        <div className="flex items-center gap-4 md:gap-7 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className={`px-2 py-0.5 rounded-full text-[8px] font-bold uppercase tracking-wider ${post.postType === 'evento' ? 'bg-blue-100 text-blue-700' : 'bg-gold/10 text-gold-muted'}`}>
                {post.postType === 'evento' ? 'Evento' : 'Post'}
              </span>
            </div>
            <h3 className="font-serif text-lg md:text-2xl font-semibold text-charcoal leading-tight mb-1 md:mb-2 truncate">{post.title}</h3>
        ...
        <div className="reveal-actions opacity-0 translate-x-4 flex gap-2 md:gap-3 transition-all duration-300 group-hover:opacity-100 group-hover:translate-x-0 shrink-0">
          <button onClick={() => onEdit(post)} className="w-10 h-10 md:w-11 md:h-11 flex items-center justify-center rounded-xl bg-alabaster text-gray-500 hover:text-gold hover:bg-white border border-eggshell hover:border-gold/30 transition-all shadow-sm cursor-pointer">
          ...
          <button onClick={() => setShowConfirm(true)} className="w-10 h-10 md:w-11 md:h-11 flex items-center justify-center rounded-xl bg-alabaster text-gray-500 hover:text-red-500 hover:bg-red-50 border border-eggshell hover:border-red-100 transition-all shadow-sm cursor-pointer">
